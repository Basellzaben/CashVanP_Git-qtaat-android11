...
public class RecVoucherSearchActivity  extends DialogFragment implements View.OnClickListener  {
   ...
   // TextView itemnm;
    ...
    private  void FillList(String s){
        ...
        String q = "Select distinct RecVoucher.DocNo,RecVoucher.TrDate,RecVoucher.Amnt,Customers.name,RecVoucher.Desc, COALESCE(RecVoucher.Post, -1)  as Post from RecVoucher left join Customers  on Customers.no =RecVoucher.CustAcc" +
                " where  RecVoucher.UserID ='"+u.toString().replaceAll("[^\\d.]", "")+"' And Customers.name like  '%"+s.toString()+"%'";
